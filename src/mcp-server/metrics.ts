...
const LIFETIME_FILE = join(SAM_DIR, "lifetime-metrics.json");

function ensureSamDir(): void {
  if (!existsSync(SAM_DIR)) mkdirSync(SAM_DIR, { recursive: true });
}

function loadLifetime(): LifetimeMetrics {
  ensureSamDir();
  const defaults: LifetimeMetrics = {
  ...
  if (!existsSync(LIFETIME_FILE)) return defaults;
  try {
    const data = JSON.parse(readFileSync(LIFETIME_FILE, "utf-8"));
...
function saveLifetime(lt: LifetimeMetrics): void {
  ensureSamDir();
  writeFileSync(LIFETIME_FILE, JSON.stringify(lt, null, 2));
}
...
export class MetricsTracker {
  private metrics: SessionMetrics = { ...EMPTY_METRICS, startTime: Date.now() };
  private lastFlushed: SessionMetrics = { ...EMPTY_METRICS };
  ...
  getMetricsSnapshot(): { tokensSaved: number; ratio: string; minutes: number } {
    const saved = this.metrics.totalOriginalTokens - this.metrics.totalCompressedTokens;
  ...
  flushToLifetime(): void {
    ...
    if (dOrig === 0 && dRead === 0 && dBash === 0 && dCompress === 0 && dCache === 0) return;
  ...
  getLifetimeSavings(): Record<string, unknown> {
    ...
    const usdSaved: Record<string, string> = {};
