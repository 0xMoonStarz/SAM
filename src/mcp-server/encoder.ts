...
export class Encoder {
   ...
   */
  compress(input: string): { compressed: string; originalTokens: number; compressedTokens: number } {
    const originalTokens = this.estimateTokens(input);
    let result = input;

    // 1. Compress tool/operation names
    for (const [full, short] of Object.entries(OPERATIONS_REV)) {
      result = result.replaceAll(full, short);
    }

    // 2. Compress bash commands (with word boundaries to avoid corrupting content)
    const bashEntries = Object.entries(BASH_SHORTCUTS).sort(
      (a, b) => b[1].length - a[1].length // longest first to avoid partial matches
    );
    ...
    const snippetEntries = Object.entries(CODE_SNIPPETS).sort(
    ...
    const pathPrefixEntries: [string, string][] = [
      ["~s", "src"],
      ["~d", "dist"],
      ["~n", "node_modules"],
      ["~p", "package.json"],
  ...
  decompress(input: string): string {
    ...
    const fileAliases = [...new Set(result.match(fileAliasRegex) || [])].sort(
      (a, b) => b.length - a.length || b.localeCompare(a)
    );
    for (const alias of fileAliases) {
      const resolved = this.dict.resolve(alias);
      if (resolved) result = result.replaceAll(alias, resolved);
    }
    ...
    const expanded = lines.map((line) => {
      ...
      if (trimmed.length > 0 && OPERATIONS[trimmed[0]]) {
        const rest = trimmed.slice(1).trim();
        return `[${OPERATIONS[trimmed[0]]}] ${rest}`;
      }
      return line;
    });
    ...
    for (const [short, full] of Object.entries(CODE_SNIPPETS)) {
    ...
    for (const [short, full] of Object.entries(BASH_SHORTCUTS)) {
