...
export class Parser {
  ...
  validate(input: string): { valid: boolean; errors: string[] } {
    ...
    for (let i = 0; i < lines.length; i++) {
      try {
        ...
        if (!node) errors.push(`L${i + 1}: empty or unparseable`);
      } catch (e) {
        errors.push(`L${i + 1}: ${(e as Error).message}`);
      }
    }
  ...
  format(ast: ASTNode): string {
    switch (ast.type) {
      ...
      case "operation": {
        let s = ast.verb || "";
        if (ast.target) s += " " + this.format(ast.target);
        if (ast.modifier) s += this.format(ast.modifier);
        return s;
      }
      case "alias":
        return ast.value || "";
  ...
  private looksLikeDeclaration(line: string): boolean {
    return /^[$&@]\w+\s*=/.test(line);
  }

  private parseDeclaration(lex: Lexer): ASTNode {
    const prefix = lex.advance(); // $ & @
    const id = lex.readWhile((c) => /\w/.test(c));
    lex.skipSpaces();
    if (lex.advance() !== "=") throw new Error("Expected '=' in declaration");
    lex.skipSpaces();
    const value = lex.remaining();
    return { type: "declaration", alias: `${prefix}${id}`, value };
  }

  private parsePipe(lex: Lexer): ASTNode {
    const ops: ASTNode[] = [];
  ...
  private parseOperation(lex: Lexer): ASTNode {
    ...
    lex.skipSpaces();
    const node: ASTNode = { type: "operation", verb };
  ...
  private parseTarget(lex: Lexer): ASTNode {
    const ch = lex.peek();
  ...
  private parseRange(lex: Lexer): ASTNode {
  ...
  private parseResponse(lex: Lexer): ASTNode {
    const ch = lex.peek();
    let status: string;
    ...
    lex.skipSpaces();
    const node: ASTNode = { type: "response", status };
