...
function installGlobalClaudeMd(quiet = false) {
    try {
        const claudeMdPath = (0, path_1.join)((0, os_1.homedir)(), ".claude", "CLAUDE.md");
        ...
        if ((0, fs_1.existsSync)(claudeMdPath)) {
            ...
            if (existing.includes(SAM_MARKER)) {
                ...
                const after = nextHeadingMatch
                    ...
                    : "";
                (0, fs_1.writeFileSync)(claudeMdPath, (before ? before + "\n\n" : "") + SAM_GLOBAL_INSTRUCTIONS + after);
                if (!quiet)
                    console.log("  ~ Updated SAM section in ~/.claude/CLAUDE.md");
            }
            else {
                (0, fs_1.writeFileSync)(claudeMdPath, existing.trimEnd() + "\n\n" + SAM_GLOBAL_INSTRUCTIONS + "\n");
                if (!quiet)
                    console.log("  + Appended SAM instructions to ~/.claude/CLAUDE.md");
            }
        }
        else {
            (0, fs_1.writeFileSync)(claudeMdPath, SAM_GLOBAL_INSTRUCTIONS + "\n");
            if (!quiet)
                console.log("  + Created ~/.claude/CLAUDE.md with SAM instructions");
        }
    }
    catch {
        if (!quiet)
            console.error("  [!!] Could not write to ~/.claude/CLAUDE.md");
    }
}
// === COMMANDS ===
